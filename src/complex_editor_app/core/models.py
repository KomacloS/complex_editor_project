...
class Subcomponent:
    ...
    # ``function_id`` is populated when the sub-component originates from the
    # Access database so that round-tripping to :mod:`complex_editor.db.mdb_api`
    # can reuse the original ``IDFunction`` value.  Buffer-only rows can leave
    # it as ``None`` and the repository will resolve the value via the macro
    # catalog.
...
class Complex:
    ...
    # ``db_id`` carries the integer ``IDCompDesc`` when the complex comes from
    # the production database.  Buffer files and ad-hoc demos can leave it as
    # ``None`` and rely solely on ``identifier``.
    db_id: int | None = None
