...
PN_RE = re.compile(r"^[A-Z0-9][A-Z0-9\-_.]{2,}$")
ALIAS_RE = re.compile(r"^[A-Za-z0-9_\-]{2,}$")
...
def validate_part_number(pn: str) -> List[FieldError]:
    ...
    if not PN_RE.match(pn):
        return [FieldError("part_number", "Part number must be alphanumeric with -_.")]
    return []
...
def validate_aliases(values: Iterable[str]) -> List[FieldError]:
    ...
    for alias in values:
        ...
        if not ALIAS_RE.match(alias):
            errors.append(FieldError("aliases", f"Alias '{alias}' contains invalid characters"))
    return errors
